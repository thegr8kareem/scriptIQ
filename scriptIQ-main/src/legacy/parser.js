 ...
 *   - TXT/MD/source code → TextDecoder
 *
 * Public API:
...
ScriptIQ.parser = (function () {
  ...
  // Source files are intentionally parsed as UTF-8 text. Keeping the list
  // explicit lets the file picker show the right files and gives ZIP uploads
  // the same support as individual uploads.
  const SUPPORTED_EXTENSIONS = [
    "pdf", "docx", "txt", "md",
    "js", "jsx", "ts", "tsx", "py", "java", "c", "h", "cpp", "cc", "hpp",
    "cs", "go", "rs", "php", "rb", "swift", "kt", "kts", "html", "htm",
    "css", "scss", "json", "xml", "yml", "yaml", "sql", "sh", "bat", "ps1",
  ];
  ...
  async function expand(file) {
    ...
    if (parseable.length === 0) {
      throw new Error(
        `"${file.name}" contains no supported text or document files` +
  ...
  async function extractText(entry) {
    ...
    if (!SUPPORTED_EXTENSIONS.includes(ext)) {
      throw new Error(`Unsupported file type ".${ext}" — use PDF, DOCX, text, or source code.`);
    }
